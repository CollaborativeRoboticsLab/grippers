...
class DynamixelServoActionNode(Node):
        ...
        *,
        ...
        try:
            ...
            self.get_logger().info(
                f"Loaded Dynamixel servo config for model={self.motor_model}, id={self._servo_config.dxl_id}, device={self._servo_config.device_name}"
            )
    ...
    def _publish_status(self) -> None:
        ...
        try:
            ...
            self._handle_position_feedback(ticks)
            position = self._servo.ticks_to_command_units(ticks)
            self.get_logger().info(f'Present position: {position} ({ticks} ticks)')
        except Exception as exc:  # noqa: BLE001
